...
- Git, Java 17+, and Maven installed
...
```bash
git clone https://github.com/gh-agents-day/Speckit-and-beyond-Java.git
cd Speckit-and-beyond-Java
mvn spring-boot:run
```
...
In Exercise 1, IDE users create a cust
...
10. Run `/agent` again to open the agent selector. You should now see **codebase-analyser** in the list. Select it, then send this prompt:
    ...
    One row per layer (Model, Repository, Service, Controller, Frontend).
...
| **Implementation**        | Used Spec Kit to generate code changes across model, service, controller, and frontend. |
